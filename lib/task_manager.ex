defmodule DOSPRJ.TaskManager do
  ...
  """
  defp splitTask(n, _, start, tasklist ) when start >n do
    tasklist
  end
  ...
  """
  defp splitTask(n, k, start, tasklist) do
    last = start + k-1
    window = Enum.to_list start..last
    task = Task.Supervisor.async(DOSPRJ.TaskSupervisor, fn -> checkPerfrectSq?(window) end)
    tasklist = [task | tasklist]
    splitTask(n, k, (start + 1),tasklist)
  end
